...
int main() {
  ...
  string op1, op2;
  int checker;
  cout << "\n\nCommands: dir - show files\nadd [filename] [file contents] - creates a new file\ndel [filename] - deletes file\ntype [filename] - show file contents\ncopy [filename] [new filename] - creates a copy of the file\nsearch [index number] - returns historial entry from the database\nquit - exits the program";
  ...
  while (command != "quit"){
    ...
    //searches through database
      ...
      cout << "\nEnter an index: ";
      if (op1.empty()){
        cout << "\nError, must include the an index number";
      }
      ...
      if (op2.empty()){
        ...
        cout << "\nError, a second operation was entered";
      }
    }//end of search
